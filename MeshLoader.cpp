...
//--------------------------------------------------------------------------------------
HRESULT CMeshLoader::Create(IDirect3DDevice9* pd3dDevice, wstring strFilename)
{
    HRESULT hr;
    ...
    for (int iMaterial = 0; iMaterial < m_Materials.size(); iMaterial++)
    {
        ...
        if (pMaterial->strTexture[0])
        {
            ...
            if (!bFound)
            {
                ...
                pMaterial->pTexture = IMAGE->AddImage(pMaterial->strTexture, pMaterial->strTexture)->texturePtr;
            }
        }
    }
...
//--------------------------------------------------------------------------------------
HRESULT CMeshLoader::LoadGeometryFromOBJ(wstring strFileName)
{
    ...
    for (; ; )
    {
        ...
        else if (0 == wcscmp(strCommand, L"mtllib"))
        {
            ...
            wstring temp = strFileName;
            int number = temp.rfind('/');
            temp = temp.substr(0, number + 1);
            strMaterialFilename = temp + strMaterialFilename;
        }
    ...
    {
        wstring mtl = strFileName;
        mtl.pop_back();
        mtl.pop_back();
        mtl.pop_back();
        mtl += L"mtl";
        V_RETURN(LoadMaterialsFromMTL(mtl));
    }
...
//--------------------------------------------------------------------------------------
HRESULT CMeshLoader::LoadMaterialsFromMTL(wstring strFileName)
{
    HRESULT hr;
    ...
    for (; ; )
    {
        ...
        else if (0 == wcscmp(strCommand, L"map_Kd"))
        {
            ...
            wstring temp = strFileName.substr(0, strFileName.rfind('/') + 1);

            pMaterial->strTexture = temp.c_str() + pMaterial->strTexture;
            pMaterial->pTexture = IMAGE->AddImage(pMaterial->strTexture, pMaterial->strTexture)->texturePtr;
        }
        else
        {
...
void CMeshLoader::InitMaterial(Material* pMaterial)
{
    ...
    pMaterial->mtl.Ambient = D3DXCOLOR(1, 1, 1, 1);
    pMaterial->mtl.Diffuse = D3DXCOLOR(1, 1, 1, 1);
    pMaterial->mtl.Emissive = D3DXCOLOR(1.f, 0.f, 0.f, 1);
    pMaterial->mtl.Specular = D3DXCOLOR(1, 1, 1, 1);
    pMaterial->mtl.Power = 1.F;
